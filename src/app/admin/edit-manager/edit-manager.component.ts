  ...
  update() {
    if (this.selected == 3){
      if(this.postForm.valid) {
        ...
        this.customerService.updateAdminComment(this.id, this.customer).subscribe(data=>{
          this.toastr.success('Update successfully!', 'System');
          this.editFinish.emit('done');
        })
      } else {
        this.toastr.error('Check your data again! ', 'System');
      }
      ...
      if(this.postForm.valid) {
        ...
        this.customerService.updateAdminOrder(this.id, this.customer).subscribe(data=>{
          this.toastr.success('Update successfully!', 'System');
          this.editFinish.emit('done');
        })
      } else {
  ...
  getCustomer() {
    this.customerService.getOne(this.id).subscribe(data => {
      ...
      this.postForm = new FormGroup({
        ...
        'selected': new FormControl(this.customer.roles[0].id),
      })
      this.image = this.customer.image;
    }, error => {
      this.toastr.error('Data access error! ', 'System');
    })
  }
